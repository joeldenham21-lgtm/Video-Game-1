extends Campfire
## The items stream's campfire, placed through build mode: a blueprint frame until its sticks and stones are
## in, dismantled with a hammer (scenes/building/campfire.tscn inherits scenes/items/campfire.tscn).


func _ready() -> void:
	super._ready()
	if BuildHooks.component(self) == null:
		BuildHooks.attach(self, &"campfire", true)
