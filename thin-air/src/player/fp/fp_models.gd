...
static func compass_mesh() -> Dictionary:
	...
	ring.add(Vector3(0, 0.001, 0), 0.024).add(Vector3(0, 0.009, 0), 0.024).add(Vector3(0, 0.011, 0), 0.022)
	b.tube(ring, 24, Vector3.FORWARD, 10.0, false, true)
	b.commit(mesh, FPMaterials.vm(&"plastic_red"))
	var face := FPMesh.Tube.new()
	face.add(Vector3(0, 0.0045, 0), 0.0215).add(Vector3(0, 0.005, 0), 0.0215)
